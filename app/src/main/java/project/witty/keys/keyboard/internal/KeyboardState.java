 ...
 * {@link #onEvent(Event,int,int,boolean,boolean)}, {@link #onFinishSlidingInput(int,int)},
 * {@link #onUpdateShiftState(int,int,boolean,boolean)}, {@link #onResetKeyboardStateToAlphabet(int,int)}.
 *
...
public final class KeyboardState {
    ...
    public interface SwitchActions {
        ...
        /**
         * Request to call back {@link KeyboardState#onUpdateShiftState(int, int,boolean,boolean)}.
         */
    ...
    public void onUpdateShiftState(final int autoCapsFlags, final int recapitalizeMode, final boolean isKeyboardShowing, final boolean isProductViewVisible) {
        if (DEBUG_EVENT) {
            Log.d(TAG, "onUpdateShiftState: " + stateToString(autoCapsFlags, recapitalizeMode));
        }
        mRecapitalizeMode = recapitalizeMode;
        updateAlphabetShiftState(autoCapsFlags, recapitalizeMode,isKeyboardShowing,isProductViewVisible);
    }
    ...
    private void updateAlphabetShiftState(final int autoCapsFlags, final int recapitalizeMode,final boolean isKeyboardShowing, final boolean isProductViewVisible) {
          if (!mIsAlphabetMode) return;
        ...
        if (!mAlphabetShiftState.isShiftLocked() && !mShiftKeyState.isIgnoring()) {
             if (mShiftKeyState.isReleasing() && autoCapsFlags != Constants.TextUtils.CAP_MODE_OFF) {
               ...
               // this condition is important to avoid setting automatic shift when product view is visible and automatic shift is activated
                if(isProductViewVisible && !isKeyboardShowing) return;
                setShifted(AUTOMATIC_SHIFT);
            } else {
                 // this condition is important to avoid setting automatic shift when product view is visible and automatic shift is activated
                 if(isProductViewVisible && !isKeyboardShowing) return;
    ...
    public void onEvent(final Event event, final int autoCapsFlags, final int recapitalizeMode,final boolean isKeyboardShowing, final boolean isProductViewVisible) {
