...
import project.witty.keys.latin.utils.TypefaceUtils;
...
public class KeyboardView extends View {
    ...
    // Round 5 FIX 14: RESTORED — mSystemBarHeight is needed for nav bar clearance on
    // edge-to-edge displays (Razr 50 gesture nav). The internal gap was a separate issue
    // caused by rowHeight being too small (fixed in FIX 15 below).
    ...
    public KeyboardView(final Context context, final AttributeSet attrs, final int defStyle) {
        ...
        // Round 5 FIX 14: RESTORED — WindowInsets listener for nav bar height
    ...
    public void setKeyboard(final Keyboard keyboard) {
        ...
        final int nightMode = getContext().getResources().getConfiguration().uiMode & android.content.res.Configuration.UI_MODE_NIGHT_MASK;
        android.util.Log.d("WK_NAVBAR", "KeyboardView.setKeyboard: mCustomColor=#" + Integer.toHexString(mCustomColor) +
                ", nightMode=" + (nightMode == android.content.res.Configuration.UI_MODE_NIGHT_YES ? "YES" : "NO"));
        invalidateAllKeys();
        requestLayout();
    }
