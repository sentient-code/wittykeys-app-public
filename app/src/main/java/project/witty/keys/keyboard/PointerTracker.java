...
import project.witty.keys.keyboard.internal.BogusMoveEventDetector;
import project.witty.keys.keyboard.internal.DrawingProxy;
import project.witty.keys.keyboard.internal.PointerTrackerQueue;
import project.witty.keys.keyboard.internal.TimerProxy;
import project.witty.keys.latin.common.Constants;
import project.witty.keys.latin.common.CoordinateUtils;
...
public final class PointerTracker implements PointerTrackerQueue.Element {
    ...
    private void callListenerOnCodeInput(final Key key, final int primaryCode, final int x,
        ...
        } else if (code == Constants.CODE_CHATGPT_CTA) {
            sListener.onCustomRequest(Constants.CODE_CHATGPT_CTA);
        } else if (code == Constants.CODE_GOOGLE_CTA) {
            sListener.onCustomRequest(Constants.CODE_GOOGLE_CTA);
        } else if (code == Constants.CODE_YOUTUBE_CTA) {
            sListener.onCustomRequest(Constants.CODE_YOUTUBE_CTA);
        } else if (code == Constants.CODE_TRANSLATION_CTA) {
            sListener.onCustomRequest(Constants.CODE_TRANSLATION_CTA);
        } else if (code == Constants.CODE_GRAMMAR_CTA) {
            sListener.onCustomRequest(Constants.CODE_GRAMMAR_CTA);
        } else if (code == Constants.CODE_TONALITY_CTA) {
            sListener.onCustomRequest(Constants.CODE_TONALITY_CTA);
        } else if (code == Constants.CODE_OCR_CTA) {
            sListener.onCustomRequest(Constants.CODE_OCR_CTA);
        } else if (code == Constants.CODE_SWITCH_EMOJI) {
                sListener.onCustomRequest(Constants.CODE_SWITCH_EMOJI);
