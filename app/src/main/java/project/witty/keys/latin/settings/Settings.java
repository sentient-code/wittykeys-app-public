...
import project.witty.keys.latin.AudioAndHapticFeedbackManager;
import project.witty.keys.latin.InputAttributes;
...
public final class Settings implements SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    public static int readKeyboardDefaultColor(final Context context) {
        ...
        // For System theme, dynamically resolve the auto-switching color
        // so it properly updates when light/dark mode changes
        if (themeId == KeyboardTheme.THEME_ID_SYSTEM) {
            return ContextCompat.getColor(context, R.color.background_lxx_system_auto);
        }

        // For other themes, use the static color array
    ...
    public static int readKeyboardColor(final SharedPreferences prefs, final Context context) {
        // For System theme, always dynamically resolve the color
        // so navbar color updates with light/dark mode changes
        final int themeId = KeyboardTheme.getKeyboardTheme(context).mThemeId;
        if (themeId == KeyboardTheme.THEME_ID_SYSTEM) {
            // Directly check night mode from system configuration to avoid context inconsistencies
            final int nightMode = context.getResources().getConfiguration().uiMode
                    & Configuration.UI_MODE_NIGHT_MASK;
            if (nightMode == Configuration.UI_MODE_NIGHT_YES) {
                return ContextCompat.getColor(context, R.color.background_lxx_dark);
            } else {
                return ContextCompat.getColor(context, R.color.background_lxx_light);
            }
        }
    ...
    public static boolean readUseMatchingNavbarColor(final SharedPreferences prefs) {
        // Round 3 Fix #6: Default to true so nav bar matches keyboard background,
        // eliminating the visible gap/seam below the keyboard.
