...
import project.witty.keys.latin.settings.RadioButtonPreference.OnRadioButtonClickedListener;
...
public final class ThemeSettingsFragment extends SubScreenFragment
    ...
    public void onCreate(final Bundle icicle) {
        ...
        // --- TEMPORARY DEBUGGING ---
        Log.d("ThemeSettings", "--- STARTING THEME POPULATION ---");

        final String[] keyboardThemeNames = res.getStringArray(R.array.keyboard_theme_names);
        final int[] keyboardThemeIds = res.getIntArray(R.array.keyboard_theme_ids);

        // Log the length of the arrays to see what the system thinks it loaded.
        Log.d("ThemeSettings", "Names array length: " + keyboardThemeNames.length);
        Log.d("ThemeSettings", "IDs array length: " + keyboardThemeIds.length);
        // --- END DEBUGGING ---

        // This loop is correct, we will leave it as is.
        for (int index = 0; index < keyboardThemeIds.length; index++) {
